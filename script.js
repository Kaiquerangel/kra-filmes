document.addEventListener('DOMContentLoaded', function () {
    ...
    const { db, collection, addDoc, getDocs, doc, updateDoc, deleteDoc, serverTimestamp, query, orderBy } = window.db;
    ...
    const aplicarFiltrosEordenacao = (listaDeFilmes) => {
        ...
        if (filtros.genero) {
            filmesProcessados = filmesProcessados.filter(f => f.genero?.some(g => g.toLowerCase().includes(filtros.genero)));
        }
        if (filtros.diretor) {
            filmesProcessados = filmesProcessados.filter(f => f.direcao?.some(d => d.toLowerCase().includes(filtros.diretor)));
        }
        if (filtros.ator) {
            filmesProcessados = filmesProcessados.filter(f => f.atores?.some(a => a.toLowerCase().includes(filtros.ator)));
        }
        
        if (filtros.ano && filtros.ano !== 'todos') {
            filmesProcessados = filmesProcessados.filter(f => f.ano.toString() === filtros.ano);
        }
