...
// Resize inteligente para a Grid - Melhora o desempenho de renderização
const getResizedUrl = (url, width = 300) => {
    if (!url || url === 'N/A') return null;
    if (url.includes('_SX')) {
        return url.replace(/_SX[0-9]+/, `_SX${width}`);
    } else if (url.includes('media-amazon.com') || url.includes('omdbapi.com')) {
        return url.replace(/\.(jpg|jpeg|png)$/i, `_SX${width}.$1`);
    }
    return url;
};
...
export const UI = {
    ...
    templates: {
        poster: (url, cssClass = 'poster-thumb', alt = 'Capa') => {
            if (url && url !== 'N/A') {
                ...
                return `
                    <img src="${finalSrc}" ${sizeAttr} loading="lazy" decoding="async" class="${cssClass}" alt="${alt}" 
                         onerror="this.style.display='none'; this.nextElementSibling.style.display='flex'">
                    <div class="${cssClass} d-flex align-items-center justify-content-center bg-dark text-muted" 
                         style="display:none !important;">
                         <i class="fas fa-film ${cssClass === 'movie-poster-img' ? 'fa-3x' : ''}"></i>
                    </div>
                `;
            }
            return `
    ...
    toggleTheme: () => {
        ...
        UI.els.themeBtn.innerHTML = isDark ? '<i class="fas fa-sun" aria-hidden="true"></i>' : '<i class="fas fa-moon" aria-hidden="true"></i>';
    ...
    renderTable: (lista, container, append = false) => {
        ...
        const emptyStateHtml = `
            <div class="d-flex flex-column align-items-center justify-content-center py-5 text-muted" style="opacity: 0.8; animation: fadeInUp 0.4s ease-out;">
                ...
                <h5 class="fw-bold">Nenhum filme por aqui!</h5>
                <p class="small mb-3">Tente ajustar seus filtros ou cadastre um novo título.</p>
            ...
            <tr data-id="${f.id}" style="cursor: pointer;">
                ...
                <td class="col-acoes align-middle text-end">
                    <div class="d-flex align-items-center justify-content-end gap-2">
                        <button class="btn btn-sm btn-link text-muted btn-detalhes" type="button" data-bs-toggle="collapse" data-bs-target="#detalhes-${f.id}" aria-label="Ver mais detalhes">
                            <i class="fas fa-chevron-down"></i>
                        </button>
                        <div class="dropdown">
                            <button class="btn btn-sm btn-outline-secondary border-0" type="button" data-bs-toggle="dropdown" aria-label="Ações">
    ...
    renderGrid: (lista, container, append = false) => {
        ...
        const cards = lista.map((f, i) => `
            <div class="movie-card" data-id="${f.id}" tabindex="0" role="button" aria-label="Filme: ${f.titulo}, Ano: ${f.ano || 'Desconhecido'}, Nota: ${(f.nota || 0).toFixed(1)}" title="${f.titulo} (${f.ano || '-'}) ★ ${(f.nota || 0).toFixed(1)}"
                 style="animation: fadeInUp 0.4s ease-out forwards; animation-delay: ${i * 0.03}s; opacity: 0;">
                <div class="position-relative" style="height: 100%;">
                    ...
                    <div class="movie-card-actions">
                        ...
                        <button class="btn-action btn-edit" aria-label="Editar ${f.titulo}" title="Editar"><i class="fas fa-ellipsis-h" aria-hidden="true"></i></button>
    ...
    clearForm: () => {
        ...
        if(notaDisplay) {
            notaDisplay.innerHTML = `<i class="fas fa-star me-1"></i>0.0`;
        }
    ...
    fillForm: (f, generosCallback, tagsCallback) => {
        ...
        const notaDisplay = document.getElementById('nota-display'); 
        if(notaDisplay) {
            notaDisplay.innerHTML = `<i class="fas fa-star me-1"></i>${parseFloat(f.nota||0).toFixed(1)}`;
        }
        ...
        if (f.assistido) {
            document.getElementById('data-assistido').value = f.dataAssistido;
        }
    ...
    updateStats: (assistidos, total) => {
        ...
        const processarEstatisticas = () => {
            ...
            let melhor = assistidos[0];
            let pior = assistidos[0];
            ...
            assistidos.forEach(f => {
                ['genero', 'atores', 'direcao'].forEach(key => { 
                    ...
                    items.forEach(i => { 
                        if(i) { 
                            const nomeNormalizado = i.trim(); 
                            counts[key][nomeNormalizado] = (counts[key][nomeNormalizado] || 0) + 1; 
                        } 
                    }); 
                });
    ...
    renderCarousel: (assistidos) => {
        ...
        if (ultimos10.length === 0) { 
            containerUltimos.innerHTML = `
                <div class="d-flex align-items-center justify-content-center w-100 p-4 text-muted border rounded" style="background: rgba(0,0,0,0.05);">
                    <i class="fas fa-history me-2"></i> Nenhum filme assistido.
                </div>`; 
        } else {
            const html = ultimos10.map((f, i) => `
                <div class="flex-shrink-0 mini-movie-card" tabindex="0" style="animation: fadeInUp 0.5s ease forwards; animation-delay: ${i * 50}ms; opacity: 0; cursor: pointer;" onclick="document.querySelector('tr[data-id=\\'${f.id}\\'] .btn-detalhes')?.click(); document.getElementById('view-btn-table').click(); document.getElementById('lista-section').scrollIntoView({behavior: 'smooth'});">
    ...
    renderCharts: (filmes) => {
        ...
        const createChart = (id, type, labels, data, labelStr, bgColors, borderColor, opts={}) => {
            ...
            if (UI.chartsInstances[id]) {
                UI.chartsInstances[id].destroy();
            }
            ...
            UI.chartsInstances[id] = new ChartJS(ctx, { 
                ...
                data: { 
                    ...
                    datasets: [{ 
                        ...
                        borderWidth: type.includes('line') ? 2 : 0, 
                        fill: true, 
                        tension: 0.3 
                    }] 
                }, 
        ...
        const count = (k) => { 
            const c={}; 
            filmes.forEach(f => (Array.isArray(f[k])?f[k]:[f[k]]).forEach(i=>{if(i)c[i]=(c[i]||0)+1})); 
        ...
        const origs={}; 
        filmes.forEach(f=>{if(f.origem)origs[f.origem]=(origs[f.origem]||0)+1}); 
        createChart('origemChart','pie',Object.keys(origs),Object.values(origs),'Origem',['#3b82f6','#14b8a6']);
        
        const anosC={}; 
        filmes.forEach(f=>{if(f.ano)anosC[f.ano]=(anosC[f.ano]||0)+1}); 
        ...
        const medAno={}; 
        filmes.forEach(f=>{if(f.ano&&f.nota){if(!medAno[f.ano])medAno[f.ano]={s:0,c:0};medAno[f.ano].s+=f.nota;medAno[f.ano].c++}}); 
        ...
        for(let i=11;i>=0;i--){
            ...
            const k=`${cap(d.toLocaleString('pt-BR',{month:'short'}).replace('.',''))}/${d.getFullYear().toString().slice(-2)}`;
            mLab.push(k);
            mapM[k]=0;
        }
    ...
    showMovieDetailModal: (f, onMarkWatched, onFetchTrailer) => {
        ...
        Swal.fire({
            ...
            `,
            showConfirmButton: !f.assistido && onMarkWatched, 
            showCancelButton: true, 
            confirmButtonText: '<i class="fas fa-check-circle me-1"></i> Marcar assistido', 
            ...
            if(res.isConfirmed && onMarkWatched) onMarkWatched(f.id); 
        });
    },
    ...
    showRandomSuggestion: (f, onMark, onRetry, onFetchTrailer) => {
        ...
        Swal.fire({
            ...
            if (res.isDismissed && res.dismiss === Swal.DismissReason.cancel && onRetry) {
                onRetry(); 
            } else if (res.isDenied && onMark) {
                onMark(f.id); 
            }
        });
    },
    ...
    initDragScroll: (el) => {
        if (!el) return;
        let isDown = false, startX, scrollLeft, animationFrameId; 
